...
class MyAgentFixedDepth(Agent):
    ...
    def act(self, state, remaining_time):
        legal_actions = self._sort_actions(state, __import__("oxono").Game.actions(state), True)

        self.start_cpu_time = __import__("time").time()
        self.time_limit = remaining_time  # pas de gestion fine

        try:
            score, move = self.max_value(state, self.max_depth, -math.inf, math.inf)
            return move
        except:
